...
public class JActivesGames extends JPanel {
	...
	private LinkedList<Game> actGames = Game.getGameActiveList();
	...
	private void setGamesList() {
		...
		actGames =Game.getGameActiveList();
		
		Integer[] tmp = new Integer[actGames.size()];
		for (int i = 0; i < tmp.length; i++) {
			tmp[i] = actGames.get(i).getID();
		}
		...
		private void setGameInfo(int gameID){
			for (Game g : actGames) {
				if(g.getID()==gameID){
					...
					String[] tmp = ManagerGamer.getGamers().toArray(new String[0]);
